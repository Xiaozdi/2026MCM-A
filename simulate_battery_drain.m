function [SOC, power_history, temp_history] = simulate_battery_drain(params, scenario, time_duration, initial_SOC)
% 使用Thevenin/Shepherd电化学模型模拟电池耗电（简化版，不考虑温度变化）
% 输入:
%   params - 参数结构体
%   scenario - 使用场景
...
% 输出:
%   SOC - 电量随时间变化 (%)
%   power_history - 功率历史记录 (W)
%   temp_history - 温度历史记录 (°C) - 恒定25°C
...
power_history = zeros(1, time_steps);
temp_history = ones(1, time_steps) * params.T_ambient;  % 恒定25°C
